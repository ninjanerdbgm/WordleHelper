...
def load_words(use_wordle_dictionary = False):
    if use_wordle_dictionary:
        with open('wordle_dictionary.json') as word_file:
...
if __name__ == '__main__':
    INCLUDE_WORDS_WITH_REPEATING_LETTERS = True
    USE_WORDLE_DICTIONARY = False
    SHOW_HELP = False
    ...
    if SHOW_HELP:
        print("First try ETNAS")
        print("If none of those letters match, try OCULI to check the three remaining vowels")

    english_words = load_words(USE_WORDLE_DICTIONARY)
    ...
    if SHOW_HELP:
        print("If, somehow, none of those match as well, here are the possible remaining words:")
        for word in possible_words_after_two_rounds_of_wordle:
            print(word)
    ...
    while not solved:
        ...
        potential_word_string = ", ".join(sorted(potential_words))
